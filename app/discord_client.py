...
class LLMCordClient(discord.Client):
    ...
    def __init__(self, config: Config):
        ...
        activity = discord.CustomActivity(
            name=(self.config.status_message or "github.com/jakobdylanc/llmcord")[:128]
        )
        ...
        # The tree attribute will be set in the setup_hook
        self.tree = None
    ...
    async def setup_hook(self):
        ...
        # Initialize the command tree now that the client is fully initialized
        ...
        async def reset(interaction: discord.Interaction):
            ...
            success = self.db.reset_user_history(interaction.user.id)
            if bool(success):
        ...
        @self.tree.command(
            name="stats", description="Show your conversation statistics with the bot"
        )
    ...
    async def on_ready(self):
        ...
        if self.config.client_id:
            logging.info(
                ...
                f"&permissions=412317273088&scope=bot%20applications.commands\n"
            )
    ...
    async def build_message_chain(
        self, start_msg: discord.Message
    ) -> Tuple[List[Dict[str, Any]], ConversationWarnings]:
        """
        ...
        if (
            not is_dm and self.user.mentioned_in(start_msg) and not start_msg.reference
        ) or (is_dm and not start_msg.reference):
        ...
        while curr_msg is not None and len(immediate_messages) < max_messages:
            ...
            async with curr_node.lock:
                ...
                if curr_node.images[:max_images]:
                    ...
                    text_content = (
                        curr_node.text[: self.config.max_text] if curr_node.text else ""
                    )
                    content = []
                    if text_content:
                ...
                if curr_node.text and len(curr_node.text) > self.config.max_text:
                    warnings.add(
                        f"⚠️ Max {self.config.max_text:,} characters per message"
                    )
                ...
                if curr_node.fetch_parent_failed or (
                    ...
                    and len(immediate_messages) == max_messages
                ):
                    warnings.add(
                        f"⚠️ Only using last {len(immediate_messages)} message{'' if len(immediate_messages) == 1 else 's'}"
                    )
        ...
        if include_history and len(messages) < max_messages:
            ...
            if not conversation_id:
                conversation_id = self.db.create_conversation(
                    user_id, guild_id, start_msg.channel.id
                )
            ...
            if history_messages:
                ...
                for msg in reversed(immediate_messages):
                    ...
                    if has_images:
                        ...
                        self.db.add_message(
                            ...
                            role,
                            text_content,
                            discord_message_id=start_msg.id,
                            has_images=True,
                        )
                    else:
                        self.db.add_message(
                ...
                warnings.add(
                    f"⚠️ Including {len(history_messages)} message(s) from previous conversation"
                )
            else:
            ...
            for msg in reversed(immediate_messages):
                ...
                if has_images:
                    ...
                    self.db.add_message(
                        conversation_id, role, content, discord_message_id=start_msg.id
                    )
    ...
    async def process_message_chain(self, message: discord.Message):
        ...
        try:
            ...
            max_message_length = (
                ...
                else (4096 - len(self.config.STREAMING_INDICATOR))
            )

            # Generate and send the response
            await self.send_llm_response(
                message, messages, warnings, use_plain_responses, max_message_length
            )
        ...
        try:
            async with original_msg.channel.typing():
                async for (
                    content_delta,
                    curr_finish_reason,
                ) in self.llm_client.generate_response(messages):
                    ...
                    if not use_plain_responses:
                        ...
                        ) and datetime.now().timestamp() - self.last_task_time >= self.config.EDIT_DELAY_SECONDS
                        msg_split_incoming = (
                        ...
                        if start_next_msg or ready_to_edit or is_final_edit:
                            ...
                            embed.description = (
                                ...
                                if is_final_edit
                                    ...
                                    response_contents[-1]
                                    + self.config.STREAMING_INDICATOR
                                )
                            )
                            embed.color = (
                                self.config.EMBED_COLOR_COMPLETE
                                if msg_split_incoming or is_good_finish
                                else self.config.EMBED_COLOR_INCOMPLETE
                            )
                ...
                if use_plain_responses and finish_reason is not None:
                    for content in response_contents:
                        if content:
                            reply_to_msg = (
                                original_msg if not response_msgs else response_msgs[-1]
                            )
                            response_msg = await reply_to_msg.reply(
