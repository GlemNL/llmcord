...
class TestSendLLMResponse:
    ...
    @pytest.mark.asyncio
    async def test_send_llm_response_plain_response(self, discord_client, mock_discord_message):
        # Setup
        ...
        with patch('app.discord_client.asyncio.create_task'):
            await discord_client.send_llm_response(
                mock_discord_message, messages, warnings, use_plain_responses, max_message_length
            )
        ...
        if len(call_args[0]) > 0:  # Positional args
            ...
            assert "This is a test response with multiple parts." in call_args[1]['content']
            assert call_args[1].get('suppress_embeds', False) is True
    
    @pytest.mark.asyncio
    async def test_send_llm_response_with_embeds(self, discord_client, mock_discord_message):
        # Setup
        messages = [{"role": "user", "content": "Hello"}]
        warnings = ConversationWarnings()
        ...
        with patch('discord.Embed', return_value=mock_embed):
            with patch('app.utils.create_embed_for_warnings', return_value=mock_embed):
                ...
                with patch('app.discord_client.asyncio.create_task', side_effect=lambda coro: coro):
    ...
    @pytest.mark.asyncio
    async def test_send_llm_response_error_handling(self, discord_client, mock_discord_message):
        # Setup
        messages = [{"role": "user", "content": "Hello"}]
        warnings = ConversationWarnings()
        use_plain_responses = False
