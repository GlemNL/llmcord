...
class TestLLMClient:
    ...
    def test_prepare_system_message(self, llm_client):
        ...
        system_msg_mistral = llm_client.prepare_system_message("mistral-medium", "mistral")
        ...
        assert "Discord IDs" in system_msg_openai["content"]  # OpenAI supports usernames
        ...
        assert "Discord IDs" not in system_msg_mistral["content"]  # Mistral doesn't support usernames
    ...
    async def test_generate_response(self, llm_client):
        ...
        # First chunk
        delta1 = ChoiceDelta(content="Hello", role=None, function_call=None, tool_calls=None)
        choice1 = Choice(delta=delta1, finish_reason=None, index=0)
        chunk1 = ChatCompletionChunk(id="1", choices=[choice1], created=1, model="openai/gpt-4o", object="chat.completion.chunk")
        mock_chunks.append(chunk1)
        
        # Second chunk
        delta2 = ChoiceDelta(content=", world!", role=None, function_call=None, tool_calls=None)
        choice2 = Choice(delta=delta2, finish_reason="stop", index=0)
        chunk2 = ChatCompletionChunk(id="2", choices=[choice2], created=2, model="openai/gpt-4o", object="chat.completion.chunk")
        ...
        mock_openai_client.chat.completions.create = AsyncMock(return_value=mock_generator())
