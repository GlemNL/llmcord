...
class TestDatabase:
    ...
    def test_add_message(self, db):
        ...
        # Execute
        result = db.add_message(conversation_id, role, content, discord_message_id, has_images)
    ...
    def test_get_active_conversation(self, db):
        ...
        conn = sqlite3.connect(db.db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT is_active FROM conversations WHERE conversation_id = ?", (conversation_id1,))
    ...
    def test_reset_user_history(self, db):
        ...
        conn = sqlite3.connect(db.db_path)
        cursor = conn.cursor()
        cursor.execute("SELECT is_active FROM conversations WHERE user_id = ?", (user_id,))
        is_active = cursor.fetchone()[0]
        conn.close()
    ...
    def test_conversation_limit(self, db):
        ...
        # Add 10 messages
        for i in range(10):
            db.add_message(conversation_id, "user" if i % 2 == 0 else "assistant", f"Message {i}")
